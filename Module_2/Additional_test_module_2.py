...
import random   
first_area = random.randrange(3, 20)  # Choose a random item from range(stop) or range(start, stop[, step])

print()
print("Как только Вы подошли к камню - потолок с шипами начал двигаться на Вас")
print(f"Вы видите, что у Вас еще есть время, смотрите на камень с числом и видите:{first_area}")

stone_range = 20

def second_area_list():
    area_list = []
    for i in range(1, stone_range): 
        #area_list2 = []                   # нашел ошибку!
        for j in range (2, stone_range):
            if first_area % (i + j) == 0 and i != j and i <= j:
                area_list.append(i)
                area_list.append(j)
            elif first_area % (i + j) != 0 or i >= j or i == j:
                continue
        #area_list.append(area_list2)
    return area_list

result = second_area_list()

no_space = ""               # тут привел оператор к str иначе в операции "+=" было бы сложение числа, а не первого значения в списке. 
for k in result:            # тут k - является порядковым номером (одновременно и числом) в списке
    no_space += str(k)      # а тут мы добавляем строковый k к списку no_space
print()
print(
      f"""
      Но Вы - лучший, у Вас, даже, есть кубок который это подтверждает 
      'Самая быстрая рука на диком западе', вы успеваете ввести шифр {no_space}
       до того, как шипы вас раздавили
      """
      )
