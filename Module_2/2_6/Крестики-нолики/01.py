def check_winner():
    ...
    if area[0][0] == area[1][1] == area[2][2] != area_symbol:               # Проверка диагоналей, после проверки - возвращает символ который стоит в ячейке 0 0
        return area[0][0]
    if area[0][2] == area[1][1] == area[2][0] != area_symbol:               # Проверка диагоналей, после проверки - возвращает символ который стоит в ячейке 0 0
        return area[0][2]
    return noone_win
...
for i in range (1, 11):
    ...
    if check_winner() == noone_win and i == 10:                                     # Прерывание, когда определился победитель
        ...
        if area[row][col] != (area_symbol):                     # Проверка на то, что была задействованна правильная ячейка
            ...
            i -= i                                              # Возвращаем значение i к моменту, когда игрок ошибся.
